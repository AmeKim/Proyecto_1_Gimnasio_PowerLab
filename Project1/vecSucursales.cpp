...
string vecSucursales::listarInstructoresPorSucursal(string codigo) {
	...
	vecInstructores* vecInstructores = &suc->obtenerVecInstructores();
    if (vecInstructores->tamanio() == 0) return "No hay instructores registrados en esta sucursal.\n";
    string resultado = "Instructores en la sucursal " + codigo + ":\n";
    for (int i = 0; i < vecInstructores->tamanio(); ++i) {
        const instructor& ins = vecInstructores->obtener(i);
